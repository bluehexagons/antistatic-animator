...
2. Enable its debug file watcher by creating the empty `debug` file in the
   Antistatic user-data directory:
   - Linux: `$XDG_DATA_HOME/antistatic/debug`, or
     `~/.local/share/antistatic/debug` when `XDG_DATA_HOME` is unset.
   - macOS: `~/Library/Application Support/antistatic/debug`.
   - Windows: `%APPDATA%/antistatic/debug`.
...
- Antistatic's current debug watcher covers character data. Stage files still
  require a game restart or a separate stage reload path.

## Implementation

- `src/storage/electron.ts` watches source files through the preload bridge.
...
- Antistatic's `app/src/utils/utils.ts` owns the debug `WatchedFile` path and
  `app/src/engine.ts` invalidates and replaces matching entities.
